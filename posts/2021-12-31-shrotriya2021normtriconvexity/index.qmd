...
image-alt: "Preview thumbnail for characterizing norm triangle inequalities via convexity post"
execute:
  echo: true
---
...
the "lengths" of individual vectors in the given vector space. A standard
...
[here](https://en.wikipedia.org/wiki/Norm_(mathematics)#Examples){.wiki}
for common examples.
...
2.  To prove the triangle inequality for $N : V \to \mathbb{R}_{\geq 0}$, the
    ...
    $$
     ...
     \text{ is convex}
    $${#eq-normtrichar-02}
    which will imply the triangle inequality for $N$ - huzzah!
...
In fact, since $N_{\ell^{p}}$ satisfies the three conditions for a norm per
